...
{jQuery(".zp-List .zpSearchLoading").addClass("show");if(jQuery("#zpSearchResultsPaging").length>0)jQuery("#zpSearchResultsPaging").empty();jQuery("#zpSearchResultsContainer").empty();zp_set_lib_searchbar_params(!1,0,!1);jQuery("input#zp-Zotpress-SearchBox-Input").autocomplete("option","source",zpSearchBarSource+zpSearchBarParams);window.zpPage=1;if(zpItemsFlag==!0&&tempCurrentTerm)
zpLastTerm=tempCurrentTerm}},response:function(event,ui)
{jQuery(".zp-List .zpSearchLoading").removeClass("show");if(ui.content=="0"||ui.content[0].label=="empty")
{if(jQuery("#zpSearchResultsPaging").length>0)jQuery("#zpSearchResultsPaging").empty();jQuery("#zpSearchResultsContainer").append("<p>No items found.</p>\n")}
else{jQuery.each(ui.content[3],function(index,item)
...
jQuery("#zpSearchResultsContainer").append(temp+"</div><!-- .zp-Entry -->\n")});if(ui.content[2].request_next!=!1&&ui.content[2].request_next!="false")
{if(zpItemsFlag==!0)
window.zpACPagination(zpItemsFlag,!1);else window.zpACPagination(zpItemsFlag,!0);zpItemsFlag=!1;zp_set_lib_searchbar_params(!1,ui.content[2].request_next,ui.content[2].request_last);jQuery("input#zp-Zotpress-SearchBox-Input").autocomplete("option","source",zpSearchBarSource+zpSearchBarParams);jQuery("input#zp-Zotpress-SearchBox-Input").autocomplete("search")}
else{window.zpACPagination(zpItemsFlag,!0);zpItemsFlag=!1}}},open:function()
